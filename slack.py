...
- If a message directly asks a question or issues a command likely meant for the bot (like asking for calculations or specific information only the bot would provide), respond with a high confidence.
...
Response: {"confidence": 90.0, "should_reply": true}

Example 2:
Message: Steve, are you okay?
Response: {"confidence": 10.0, "should_reply": false}

Example 3:
Message: Who are you?
Response: {"confidence": 95.0, "should_reply": true}  // The bot is the only non-human entity in the channel, so any direct query like this should be perceived as directed towards the bot.
...
Response: {"confidence": 15.0, "should_reply": false}  // Analyze if the message is discussing the bot's functionality or settings without directly engaging with it.
...
def handle_message(event_data: dict[str, Any]) -> None:
    ...
    if message.get("subtype") is None and "bot_id" not in message:
        ...
        if should_reply:
            bot_response = main_bot.generate_response(user_message)
